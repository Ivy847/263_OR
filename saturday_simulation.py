...
def generate_saturday_demand(stores, demand_limits):
    ...
    for store in stores:
        demand = round(np.random.uniform(low=0.0, high=demand_limits[store], size=None))
        if demand != 0:
...
def bootstrap_weekday_demand(store_demands):
    ...
    for store in store_demands:
        weekday_demand_dict[store] = np.random.choice(store_demands[store])
...
routes = [
    ["CentrePort Wellington", "Woolworths Johnsonville Mall", "Woolworths Aotea", "Woolworths Tawa", "CentrePort Wellington"],
    ["CentrePort Wellington", "Woolworths Petone", "Woolworths Wainuiomata", "Woolworths Lower Hutt", "Woolworths Upper Hutt", "CentrePort Wellington"],
]
...
# creates matrix of durations between points
...
################################ SIMULATE DEMAND #######################################
demands = generate_saturday_demand(store_names, weekday_demand_distribution)
